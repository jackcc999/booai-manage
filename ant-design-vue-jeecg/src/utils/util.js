...
const publicKey = 'MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCxGJm1AmuCpuF4FAqM6IS5SOZCKzPBJaPG6raFCvseZUx/53K5jHEbiwVJN5NMgh/BuZ2TTgmQPIDJHHACIcgDOXzd6Zx0yxSik/q1L9575vKlhkljtmexlYakcKtHt1bK2qKyK0fJNq+HJNT0vN8w0PNKglDVRuJKVL2ottS4xQIDAQAB';
...
export function getTimeNonceStr() {
  var date = new Date();

  var year = date.getFullYear();          // 获取完整的年份(4位,1970-????)
  var month = date.getMonth() + 1;        // 获取当前月份(0-11,0代表1月)
  var day = date.getDate();               // 获取当前日(1-31)
  var minisec = date.getMilliseconds();   // 获取当前毫秒数(0-999)
  var hour = date.getHours();             // 获取当前小时数(0-23)
  var minute = date.getMinutes();         // 获取当前分钟数(0-59)
  var sec = date.getSeconds();            // 获取当前秒数(0-59)
  return year + "" + (month < 10 ? "0" + month : month) + (day < 10 ? "0" + day : day) + (hour < 10 ? "0" + hour : hour) + (minute < 10 ? "0" + minute : minute) + (sec < 10 ? "0" + sec : sec) + minisec;
}
...
export function formatDateTime(timestamp) {
  ...
  var minute = date.getMinutes();         // 获取当前分钟数(0-59)
  var sec = date.getSeconds();            // 获取当前秒数(0-59)
  return year + "-" + (month < 10 ? "0" + month : month) + "-" + (day < 10 ? "0" + day : day)  + " " + (hour < 10 ? "0" + hour : hour) + ":" + (minute < 10 ? "0" + minute : minute) + ":" + (sec < 10 ? "0" + sec : sec);
...
export function filterSelectOption (value, option) {
...
export function rsaEncrypt (content) {
  ...
  // 返回加密后结果
  return jsRsa.encrypt(content).replaceAll('+', '-').replaceAll('/', '_').replaceAll('=', '');
}
