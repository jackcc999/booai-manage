<template>
  <div>
    <a-form-model ref="form" :model="model" :rules="validatorRules" class="password-retrieval-form" @keyup.enter.native="nextStep">
      <a-form-model-item label="手机号码" required prop="phone" :labelCol="{span: 5}" :wrapperCol="{span: 19}">
        <a-input v-model="model.phone" type="text" autocomplete="false" placeholder="请输入手机号码">
          <a-icon slot="prefix" type="phone" :style="{ color: 'rgba(0,0,0,.25)'}"/>
        </a-input>
      </a-form-model-item>
      <a-form-model-item v-if="show" required prop="captcha" label="短信验证码" :labelCol="{span: 5}" :wrapperCol="{span: 19}">
        ...
        <a-button type="link" style="position: absolute;right: -7px;top: -6px;" tabindex="-1" size="default" :disabled="state.smsSendBtn" @click.stop.prevent="getCaptcha" v-text="!state.smsSendBtn && '获取验证码' || ('剩余 ' + state.time + ' 秒')"></a-button>
      </a-form-model-item>
