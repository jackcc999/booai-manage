  ...
  <div>
    <a-form-model ref="form" :model="model" :rules="validatorRules" class="password-retrieval-form">
      ...
      <a-form-model-item label="账号名" v-bind="layout">
        <a-select v-model="username">
          <a-select-option v-for="item in userList.users" :key="item.username">{{item.username}}（{{item.realname}}）</a-select-option>
        </a-select>
      </a-form-model-item>
...
//    },
    ...
    data () {
      return {
        ...
        loading: false,
        username: this.userList.users[0].username,
        validatorRules: {
          password: [{
