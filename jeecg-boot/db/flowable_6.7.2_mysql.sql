...
VALUES ('1', 'flowable', 'org/flowable/eventregistry/db/liquibase/flowable-eventregistry-db-changelog.xml',
        '2022-03-12 08:37:27', 1, 'EXECUTED', '8:1b0c48c9cf7945be799d868a2626d687',
...
VALUES ('2', 'flowable', 'org/flowable/eventregistry/db/liquibase/flowable-eventregistry-db-changelog.xml',
        '2022-03-12 08:37:28', 2, 'EXECUTED', '8:0ea825feb8e470558f0b5754352b9cda',
        'addColumn tableName=FLW_CHANNEL_DEFINITION; addColumn tableName=FLW_CHANNEL_DEFINITION', '', NULL, '4.5.0',
        NULL, NULL, '7074247706');
INSERT INTO `FLW_EV_DATABASECHANGELOG`
VALUES ('3', 'flowable', 'org/flowable/eventregistry/db/liquibase/flowable-eventregistry-db-changelog.xml',
        '2022-03-12 08:37:28', 3, 'EXECUTED', '8:3c2bb293350b5cbe6504331980c9dcee', 'customChange', '', NULL, '4.5.0',
        NULL, NULL, '7074247706');
COMMIT;
...
(
    ...
    `category_name` varchar(64)  NULL DEFAULT '' COMMENT '流程分类名称',
    `code`          varchar(64)  NULL DEFAULT '' COMMENT '分类编码',
...
) ENGINE = InnoDB AUTO_INCREMENT = 1638390725738336259  COMMENT = '流程分类表' ROW_FORMAT = DYNAMIC;
...
(
    ...
    `title`           varchar(255)  NULL DEFAULT '' COMMENT '抄送标题',
    `process_id`      varchar(64)  NULL DEFAULT '' COMMENT '流程主键',
    `process_name`    varchar(255)  NULL DEFAULT '' COMMENT '流程名称',
    `category_id`     varchar(255)  NULL DEFAULT '' COMMENT '流程分类主键',
    `deployment_id`   varchar(64)  NULL DEFAULT '' COMMENT '部署主键',
    `instance_id`     varchar(64)  NULL DEFAULT '' COMMENT '流程实例主键',
    `task_id`         varchar(64)  NULL DEFAULT '' COMMENT '任务主键',
    `user_id`         bigint NULL DEFAULT NULL COMMENT '用户主键',
    `originator_id`   bigint NULL DEFAULT NULL COMMENT '发起人主键',
    `originator_name` varchar(64)  NULL DEFAULT '' COMMENT '发起人名称',
    `create_by`       varchar(64)  NULL DEFAULT '' COMMENT '创建者',
    `create_time`     datetime NULL DEFAULT NULL COMMENT '创建时间',
    `update_by`       varchar(64)  NULL DEFAULT '' COMMENT '更新者',
    `update_time`     datetime NULL DEFAULT NULL COMMENT '更新时间',
...
) ENGINE = InnoDB  COMMENT = '流程抄送表' ROW_FORMAT = DYNAMIC;
...
(
    ...
    `form_name` varchar(64)  NULL DEFAULT '' COMMENT '表单名称',
    `node_name` varchar(255)  NULL DEFAULT '' COMMENT '节点名称',
    `content`   longtext  NULL COMMENT '表单内容',
    PRIMARY KEY (`deploy_id`, `form_key`, `node_key`) USING BTREE
) ENGINE = InnoDB  COMMENT = '流程实例关联表单' ROW_FORMAT = DYNAMIC;
...
-- ----------------------------
-- Table structure for wf_flow_biz
-- ----------------------------
DROP TABLE IF EXISTS `wf_flow_biz`;
CREATE TABLE `wf_flow_biz`
(
    `id`              varchar(32)   NOT NULL COMMENT 'ID',
    `biz_name`        varchar(100)  NOT NULL COMMENT '业务名称',
    `table_name`      varchar(255)  NOT NULL COMMENT '数据库表名',
    `form_url`        varchar(255)  NULL DEFAULT NULL COMMENT '业务表单地址（流程审核时看到的页面）',
    `form_url_mobile` varchar(255)  NULL DEFAULT NULL COMMENT '业务表单移动端地址（流程审核时看到的页面）',
    `title_exp`       varchar(300)  NULL DEFAULT NULL COMMENT '标题表达式（参考：XXXX【${busname}】-XXXX【${name}】；其中${}表达式取流程变量的值）',
    `status_api_url`  varchar(300)  NULL DEFAULT NULL COMMENT '状态变更API地址（流程状态改变时，调用业务模块api地址进行状态变更）',
...
) ENGINE = InnoDB  COMMENT = '流程业务表' ROW_FORMAT = DYNAMIC;

-- ----------------------------
-- Records of wf_flow_biz
-- ----------------------------

-- ----------------------------
-- Table structure for wf_flow_biz_deploy
-- ----------------------------
DROP TABLE IF EXISTS `wf_flow_biz_deploy`;
CREATE TABLE `wf_flow_biz_deploy`
(
    `id`             varchar(32)   NOT NULL COMMENT 'ID',
    `table_name`     varchar(255)  NOT NULL COMMENT '流程业务表名',
    `model_key`      varchar(255)  NOT NULL COMMENT '流程key',
    `model_name`     varchar(255)  NULL DEFAULT NULL COMMENT '流程名称',
    `category`       varchar(20)   NOT NULL COMMENT '适用范围：1 所有租户，2 指定租户',
    `tenant_id`      varchar(32)  NULL DEFAULT NULL COMMENT '租户id',
    `deploy_id`      varchar(64)   NOT NULL COMMENT '已发布的流程实例主键',
    `process_def_id` varchar(64)   NOT NULL COMMENT '已发布的流程定义id',
    `version`        int NULL DEFAULT NULL COMMENT '流程版本',
    `flow_biz_id`    varchar(32)  NULL DEFAULT NULL COMMENT '流程业务ID',
    `create_by`      varchar(100)  NULL DEFAULT NULL COMMENT '创建人',
    `create_time`    datetime NULL DEFAULT NULL COMMENT '创建时间',
    `update_by`      varchar(100)  NULL DEFAULT NULL COMMENT '更新人',
    `update_time`    datetime NULL DEFAULT NULL COMMENT '更新时间',
    PRIMARY KEY (`id`) USING BTREE,
    UNIQUE INDEX `deploy_key`(`table_name` ASC, `model_key` ASC) USING BTREE
) ENGINE = InnoDB  COMMENT = '流程业务关联表' ROW_FORMAT = DYNAMIC;

-- ----------------------------
-- Records of wf_flow_biz_deploy
-- ----------------------------
...
) ENGINE = InnoDB AUTO_INCREMENT = 1623613716473737218  COMMENT = '流程表单信息表' ROW_FORMAT = DYNAMIC;
